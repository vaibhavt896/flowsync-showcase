  ...
  useEffect(() => {
    ...
    const initializeSoundSystem = async () => {
      try {
        ...
        console.warn('Sound system initialization failed (non-critical):', error)
      }
    }
    ...
    const initializePremiumSystems = async () => {
      try {
        ...
        const { getExtensionSystem } = await import('@/services/extensionSystem')
        ...
        console.log('Premium systems initialized successfully')
      } catch (error) {
        console.warn('Premium systems initialization failed (non-critical):', error)
      }
    }
    ...
    <QueryClientProvider client={queryClient}>
      ...
      <LivingInterfaceProvider>
        <ParticleSystemProvider 
          options={{
            count: 25,
            colors: ['oklch(0.65 0.31 240)', 'oklch(0.68 0.29 290)', 'oklch(0.72 0.25 195)', 'oklch(0.75 0.28 155)'],
            size: { min: 1, max: 6 },
